...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    ...
    console.warn("[webhook cobranca] corpo não-JSON")
  ...
  for (const evento of eventos) {
    ...
    if (!codigoSolicitacao || !situacao) {
      console.warn("[webhook cobranca] evento sem codigoSolicitacao/situacao", JSON.stringify(evento).slice(0, 300))
      continue
    }
    ...
    console.info(`[webhook cobranca] ${codigoSolicitacao} -> ${situacao} (${count} atualizada)`)
  }
...
export function loader() {
  return Response.json({ ok: true, webhook: "cobranca" })
}
